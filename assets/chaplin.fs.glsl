...
vec3 adjust_s(vec3 rgb, vec3 adjustValue, mediump float adjustRatio) {
    ...
    if (increment > 0.) {
        ...
        color_new = vec3(-1.0 * l * 255.0 * increment + (1.+increment) * color.r,
                         -1.0 * l * 255.0 * increment + (1.+increment) * color.g,
                         -1.0 * l * 255.0 * increment + (1.+increment) * color.b);
...
lowp vec3 color1 = adjustHueSaturation(color0, vec3(0.00, -1.00, 0.00),2.0);
lowp vec3 color2 = adjustHueSaturation(color1, vec3(0.00, 0.00, 0.53),0.166667);
lowp vec3 color3 = adjustHueSaturation(color2, vec3(0.00, 0.00, 0.75),0.333333);
lowp vec3 color4 = adjustHueSaturation(color3, vec3(0.00, 0.00, 0.39),0.5);
lowp vec3 color5 = adjustHueSaturation(color4, vec3(0.00, 0.00, 0.69),0.666667);
lowp vec3 color6 = adjustHueSaturation(color5, vec3(0.00, 0.00, 0.81),0.0);
lowp vec3 color7 = adjustHueSaturation(color6, vec3(0.00, 0.00, 0.04),0.833333);
lowp vec3 color8 = adjustCurve(color7, inputImageTexture2);
oColor = vec4(color8, textureColor.a);
}
