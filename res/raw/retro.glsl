...
void main() {
    ...
    mediump vec3 color1 = adjustHueSaturation(textureColor.rgb, vec3(0.0, 0.03, 0.0), 6);
    mediump vec3 color2 = adjustHighlightShadow(color1, 0.1, 0.94);
    mediump vec3 color3 = adjustVibrance(color2, 0.7);
    mediump vec3 color4 = adjustSaturation(color3, 1.7);
    mediump vec3 color5 = adjustColorBalance(color4,
                                          vec3(0.0, 0.0, -0.2),
                                          vec3(0.02, -0.03, -0.35),
                                          vec3(0.03, 0.05, 0.09),
                                          true);
    
    mediump vec3 color6 = adjustSelectiveColorBlack(color5, vec4(0.1, 0.06, 0.05, -0.05), 6);
    mediump vec3 color7 = adjustSelectiveColorGray(color6, vec4(0.04, 0.0, 0.0, -0.1), 8);
    mediump vec3 color8 = adjustSelectiveColorWhite(color7, vec4(0, 0.0, 0.2, -0.01), 7);
    mediump vec3 color9 = adjustSelectiveColorRGB(color8, vec4(-0.06, 0.3, 0.0, -0.06), 1);
    mediump vec3 color10 = adjustSelectiveColorCMY(color9, vec4(0.1, 0.03, 0.24, 0.0), 3);
    
    mediump vec3 color11 = adjustCurve(color10);
