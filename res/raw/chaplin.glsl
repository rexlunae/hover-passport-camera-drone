...
lowp vec3 adjustCurve2(vec3 color)
{
    lowp vec3 newColor = clamp(color, 0.0001, 1.0);
    
    lowp float redCurveValue = texture2D(inputImageTexture3, vec2(newColor.r, 0.0)).r;
    lowp float greenCurveValue = texture2D(inputImageTexture3, vec2(newColor.g, 0.0)).g;
    lowp float blueCurveValue = texture2D(inputImageTexture3, vec2(newColor.b, 0.0)).b;
...
void main() {
    ...
    mediump vec3 color1 = adjustCurve(textureColor.rgb);
    mediump vec3 color2 = adjustHueSaturation(color1, vec3(0., 0., 0.12), 6);
    
    mediump vec3 color3 = adjustVibrance(color2, -0.56);
    mediump vec3 color4 = adjustSaturation(color3, 0.);
    mediump vec3 color5 = adjustCurve2(color4);
